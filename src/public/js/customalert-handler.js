document.addEventListener("DOMContentLoaded", () => {
  ...
  const openModal = (options) => {
    activeFormId = options.formId;
    activeTargetUrl = options.href;
  ...
  // Menggunakan Event Delegation untuk menangkap elemen .btn-confirm secara global
  document.body.addEventListener("click", (e) => {
    const targetBtn = e.target.closest(".btn-confirm");
    if (!targetBtn) return;

    // Matikan aksi default submit HTML dan intervensi library lain
    e.preventDefault();
    e.stopPropagation();

    openModal({
      formId: targetBtn.getAttribute("data-form-id"),
      href: targetBtn.getAttribute("href"),
